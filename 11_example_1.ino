...
void loop() {
 ...
 if (dist_raw < _DIST_MIN) {
    ...
    dist_raw=dist_prev;
     ...
     if(dist_ema-10 < _DIST_MIN){digitalWrite(PIN_LED,1);}
     else if(dist_ema+10>_DIST_MAX){digitalWrite(PIN_LED,1);}
     else{digitalWrite(PIN_LED,0);}
    
  // Apply ema filter here  
  dist_prev=dist_raw;
  dist_ema = _EMA_ALPHA*dist_raw+_EMA_ALPHA *dist_ema;
  // adjust servo position according to the USS read value

  // add your code here!
  // Use _TARGET_LOW, _TARGTE_HIGH
  if(dist_ema<_TARGET_LOW){myservo.writeMicroseconds(_DUTY_MIN);}
  else if(dist_ema>_TARGET_HIGH){myservo.writeMicroseconds(_DUTY_MAX);}
  else{myservo.writeMicroseconds((200/18)*(dist_ema-180)+500);}
